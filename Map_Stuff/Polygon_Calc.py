...
def color_polygon(polygone, size=5, poly_class=1, im_mask=None,):
    """
    Fills the polygone `polygone` with values `poly_class` leaving holes uncolored
    Returns the image with the colored polygones
    :param polygone: the polygone to be colored
    ...
    XYs = [(x, y) for x in range(left, right+1) for y in range(down, up+1) if Point(x, y).intersects(polygon)]
