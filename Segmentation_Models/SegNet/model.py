...
# modules
from Utils import _variable_with_weight_decay, _variable_on_cpu, _add_loss_summaries, _activation_summary, print_hist_summery, get_hist, per_class_acc, writeImage, get_certainity
from Inputs import *
...
def training(FLAGS, is_finetune=False):
  ...
  with tf.Graph().as_default():
    ...
    loss, eval_prediction = inference(FLAGS.num_class, train_data_node, train_labels_node, batch_size, phase_train, use_weights)
    ...
    with tf.Session() as sess:
      ...
      for step in trange(startstep+total_steps, startstep + max_steps+total_steps, desc='training', leave=True):
        ...
        if step % EPOCH_ITER*EPOCHS_UNTIL_VAL == 0:
          ...
          if total_val_loss > last_val_loss:
              ...
              if val_not_imp_t >= PATIENCE:
                  print("Terminated Training, Best Model (at step %d) saved %d validations ago" % (best_model_step, PATIENCE))
