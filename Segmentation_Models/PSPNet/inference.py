...
def main():
    ...
    # in inference immer alles restoren
    restore_var = [v for v in tf.global_variables()]
    # dont restore last layer! different shape cause osm classes = 6 !needs to be finetuned with osm data!
    ...
    delta = b-a
    print('Time required for inference: ',delta)

    # plot
    plt.figure(1, [15, 30])
    plt.subplot(121)
   # plt.imshow(img) todo add orig image
    plt.axis('off')
    plt.subplot(122)
    ...
    misc.imsave(args.save_dir+'/prediction_berlin3.png', preds[0])
