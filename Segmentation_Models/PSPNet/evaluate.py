...
osm_param = {'crop_size': [720, 720],
                    'num_classes': 6,
                    'ignore_label': 255,  # ignore nothing
                    'num_steps': 30,  # test value
                    'model': PSPNet101,
                    'data_dir': '../../ANN_DATA/GoogleImages_17_600x600_cropped',
                    'val_list': './list/osm_test_list.txt'}
def get_arguments():
...
def main():
    ...
    pred_gt = decode_labels(raw_output_up, shape, num_classes)
    # Calculate mIoU
    ...
    for step in trange(num_steps, desc='evaluation', leave=True):
       fn1, fn2 = file.readline().split('\n')[0].split(' ')
       f1 = os.path.join(data_dir, fn1)
       f2 = os.path.join(data_dir, fn2)
       print(f1)
     #  f1 = "./input/osm.png"
     #  f2 = "./input/gt_osm.png"
       preds = sess.run([update_op, pred_gt], feed_dict={image_filename: f1, anno_filename: f2})
       misc.imsave(args.save_dir+ 'Pred_'+str(step)+'.png', preds[1][0]) # zweiter tensor in list [1], erster in decode_labels [0]
    #preds = sess.run(pred_gt, feed_dict={image_filename: f1, anno_filename: f2})
