...
## THX to MrZhousf
## https://blog.csdn.net/zsf442553199/article/details/82217717
...
def acc(labels,
    ...
    with variable_scope.variable_scope(name, 'acc',
                                       (predictions, labels, weights)):
        # Check if shape is compatible.
        predictions.get_shape().assert_is_compatible_with(labels.get_shape())
        ...
        def compute_acc(name):
            ...
            acc = math_ops.div(cm_diag, sum_over_row)
            denominator = sum_over_row + sum_over_col - cm_diag
            # The mean is only computed over classes that appear in the
            # label or prediction tensor. If the denominator is 0, we need to
            # ignore the class.
        ...
        acc_v = compute_acc('acc')

        if metrics_collections:
            ops.add_to_collections(metrics_collections, acc_v)

        if updates_collections:
            ops.add_to_collections(updates_collections, update_op)
        return acc_v, update_op
