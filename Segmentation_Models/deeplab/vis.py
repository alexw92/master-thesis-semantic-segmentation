...
flags.DEFINE_string('save_dir', None, 'Where to save predictions')

flags.DEFINE_enum('colormap_type', 'osm', ['pascal', 'cityscapes', 'osm','osm_nores', 'kaggle_dstl', 'vaihingen'],
...
# The format to save prediction
_PREDICTION_FORMAT_NC = '%06d_nc_prediction'

# The format to save prediction
_Label_FORMAT_NC = '%06d_nc_label'
...
def _process_batch(sess, original_images, semantic_predictions, image_names,
                   ...
                   raw_save_dir, labels, train_id_to_eval_id=None):
  ...
  for i in range(num_image):
    ...
    save_annotation.save_annotation(
        label, save_dir,
        _Label_FORMAT % (image_id_offset + i), add_colormap=True,
        colormap_type=FLAGS.colormap_type)

    # Save prediction.
    save_annotation.save_annotation(
        crop_semantic_prediction, save_dir,
        _PREDICTION_FORMAT_NC % (image_id_offset + i), add_colormap=False)

    # Save groundtruth
    save_annotation.save_annotation(
        label, save_dir,
        _Label_FORMAT_NC % (image_id_offset + i), add_colormap=False)
