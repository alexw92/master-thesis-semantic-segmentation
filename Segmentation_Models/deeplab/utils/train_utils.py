...
def add_softmax_cross_entropy_loss_for_each_scale(scales_to_logits,
  ...
  for scale, logits in six.iteritems(scales_to_logits):
    ...
    loss_weight = classweights
    weighted_logits = tf.multiply(tf.reshape(logits, shape=[-1, num_classes]), loss_weight)
...
def get_model_init_fn(train_logdir,
  ...
  tf.logging.info('Initializing model from path: %s (Last Layer: '+str(initialize_last_layer)+')', tf_initial_checkpoint)
