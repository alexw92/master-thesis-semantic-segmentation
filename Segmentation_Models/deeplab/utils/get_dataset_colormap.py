...
_PASCAL = 'pascal'
_OSM = 'osm'
_OSM_NORES = 'osm_nores'
_KAGGLE_DSTL = 'kaggle_dstl'
_VAIHINGEN = 'vaihingen'
...
def create_osm_nores_label_colormap():
  return np.asarray([
        [0, 0, 0],
        [213, 131, 7],
        [0, 153, 0],
...
def create_kaggle_label_colormap():
  """Creates a label colormap used in CITYSCAPES segmentation benchmark.

  Returns:
    A colormap for visualizing segmentation results.
  """
  return np.asarray([(0, 0, 0),    # 0 = OTHER
...
def create_vaihingen_label_colormap():
  """Creates a label colormap used in CITYSCAPES segmentation benchmark.
  ...
  Car = [255, 255, 102]
  Clutter = [200, 0, 0]
  return np.array([Impervious_surfaces, Buildings, vegetation, Tree, Car, Clutter])
...
def get_osm_nores_name():
    return _OSM_NORES


def get_kaggle_dstl_name():
    return _KAGGLE_DSTL

def get_vaihingen_dstl_name():
    return _VAIHINGEN
...
def create_label_colormap(dataset=_PASCAL):
  ...
  if dataset == _ADE20K:
    ...
    return create_pascal_label_colormap()
      ...
      return create_osm_label_colormap()
  elif dataset == _KAGGLE_DSTL:
      return create_kaggle_label_colormap()
  elif dataset == _VAIHINGEN:
      return create_vaihingen_label_colormap()
  elif dataset == _OSM_NORES:
      return create_osm_nores_label_colormap()
  else:
...
def label_to_color_image(label, dataset=_PASCAL):
  ...
  if np.max(label) >= _DATASET_MAX_ENTRIES[dataset]:
    print(np.max(label))
    print(_DATASET_MAX_ENTRIES[dataset])
